...
from transformers_neuronx.olmo.modules import OlmoForCausalLM
from transformers_neuronx.olmo.hlo import OlmoForSamplingNoEmbeddingHlo


class OlmoForSampling(base.NeuronModelBase):
    ...
    def __init__(self, config, *, n_positions=2048, batch_size=1, amp='f32', tp_degree=2,
        ...
        config = OlmoConfig(config, n_positions, batch_size, amp, tp_degree)
        super().__init__(OlmoForCausalLM, config)
        ...
        hlo_builder = OlmoForSamplingNoEmbeddingHlo(config, neuron_config=self.neuron_config)
