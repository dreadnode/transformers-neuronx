...
from transformers_neuronx.olmo.config import OlmoConfig

class OlmoForCausalLM(module.PretrainedModel):
...
class OlmoModel(module.LowMemoryModule):

    def __init__(self, config: OlmoConfig):
        super().__init__()
        self.embed_tokens = module.LowMemoryEmbedding(config.vocab_size, config.hidden_size)
        self.layers = module.LowMemoryModuleList([OlmoDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = module.LowMemoryLayerNorm(config.hidden_size, elementwise_affine=False)


class OlmoDecoderLayer(module.LowMemoryModule):

    def __init__(self, config: OlmoConfig):
        super().__init__()
        self.self_attn = OlmoAttention(config)
        self.mlp = OlmoMLP(config)
        self.input_layernorm = module.LowMemoryLayerNorm(config.hidden_size, elementwise_affine=False)
        self.post_attention_layernorm = module.LowMemoryLayerNorm(config.hidden_size, elementwise_affine=False)


class OlmoAttention(module.LowMemoryModule):
...
class OlmoMLP(module.LowMemoryModule):
